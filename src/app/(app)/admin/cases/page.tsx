    ...
    try {
      for (const yearFolder of MOCK_GOOGLE_DRIVE_ROOT) {
        ...
        for (const clientFolder of yearFolder.children) {
          ...
          let clientObject = MOCK_CLIENTS_DB[clientKey];
          ...
          if (!clientObject) {
            ...
            newClientsCount++;
            currentSyncedItems.push(`    Added new client: ${clientName}`);
          } else {
            updatedClientsCount++;
            currentSyncedItems.push(`    Updating existing client: ${clientName}`);
          }
          setSyncedItemsLog([...currentSyncedItems]);
            ...
            sessionStrings.forEach((sessionStr, index) => {
              ...
              if (!existingSession && parsedSession.content && parsedSession.sessionNumber && parsedSession.dateOfSession) {
                const newSession: SessionNote = {
                  id: `sess-${clientKey}-${Date.now()}-${index}`,
                  clientId: clientKey,
                  sessionNumber: parsedSession.sessionNumber || MOCK_SESSIONS_DB[clientKey].length + 1,
                  dateOfSession: parsedSession.dateOfSession,
                  attendingClinicianId: parsedSession.attendingClinicianId!,
                ...
                MOCK_SESSIONS_DB[clientKey].push(newSession);
                MOCK_SESSIONS_DB[clientKey].sort((a,b) => new Date(b.dateOfSession).getTime() - new Date(a.dateOfSession).getTime());
                newSessionsCount++;
                currentSyncedItems.push(`      Added session #${newSession.sessionNumber} for ${clientName}`);
                setSyncedItemsLog([...currentSyncedItems]);
              }
            });
          }
