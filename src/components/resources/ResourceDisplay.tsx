...
import { CalendarDays, UserCircle, Paperclip, Eye, FileText, Image as ImageIcon, Video, FileArchive, LinkIcon, Package as PackageIcon } from 'lucide-react';
        ...
        <div className="text-sm text-muted-foreground flex flex-wrap items-center gap-x-4 gap-y-1 mt-2">
          <div className="flex items-center gap-2">
            <Avatar className="h-8 w-8">
              <AvatarImage src={`https://picsum.photos/seed/${resource.authorId}/32/32`} alt={resource.authorName} data-ai-hint="author photo"/>
          ...
          <div className="flex items-center gap-1">
            ...
            <span>Published: {format(new Date(resource.publishedAt || resource.createdAt), 'PPP')}</span>
          </div>
        ...
        {resource.externalLink && (
            <div className="mb-6 p-4 border border-dashed border-primary/50 rounded-md bg-primary/5">
                <h3 className="text-md font-semibold mb-2 text-primary flex items-center gap-2">
                    <LinkIcon className="h-5 w-5" /> External Resource Link
                </h3>
      ...
      {resource.attachments && resource.attachments.length > 0 && (
        <CardFooter className="border-t pt-6 flex-col items-start">
            ...
            {resource.attachments.map((att) => (
                ...
                <div className="flex items-center gap-2">
                    {/* For attachments, a direct URL is often for download, preview might be different */}
